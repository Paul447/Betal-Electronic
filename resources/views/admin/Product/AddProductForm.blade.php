            ...
            <div class="col col-md-10 card p-0 mt-4 d-inline-block  border border-1 shadow-sm overflow-auto boxxx">
                ...
                <form action="{{ $url }}" method="post" class="p-4" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3 main-Wrapper" id="output">
                        ...
                        {{-- <div class="col col-12 col-lg-6 col-sm-12 col-md-12 col-xl-4 mb-3" id="output" ><label for="recipient-name" class="col-form-label"
                            >Select Varition</label>
                             <select class="form-select" name="Varition[]" aria-label="Default select example" onchange="fetchVaritionOption(this.value)">
                              <option selected >Choose The Menu</option> 
                              @foreach ($variatioon as $variatioon)
                              <option value="{{ $variatioon->variation_id }}">{{ $variatioon->variation_name }}</option>
                          @endforeach
                             </select>
                        </div>--}}
                    </div> 
                    ...
                    .then(response => {
                        let html = ` <div class="col col-12 col-lg-6 col-sm-12 col-md-12 col-xl-4 mb-3" id="output" ><label for="recipient-name" class="col-form-label"
                                >Varition Value</label>
                                 <select class="form-select" name="varitionValue[]" aria-label="Default select example">
                                  <option selected >Choose The Menu</option> `;
                        for (const x of response.varOptValue) {
                            html += ` <option value="${x.variationoption_id}">${x.value}</option>`
                        }
                        console.log(response);
